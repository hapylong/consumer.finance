...
public class InterestController {
    ...
    public Object deleteById(HttpServletRequest request, HttpServletResponse response) {
        ...
        logger.info("利息计划删除:" + JSONObject.toJSONString(params));
        try {
            iterestService.deleteById(params);
            result.put("retCode", AccountConstant.SUCCESS);
            result.put("retMsg", "利息计划删除成功");
        } catch (BizException e) {
            logger.error("利息计划删除异常", e);
            result.put("retCode", AccountConstant.ERROR);
            result.put("retMsg", "利息计划删除异常->" + e.getMsg());
        } catch (Exception e) {
            logger.error("利息计划删除异常", e);
            result.put("retCode", AccountConstant.ERROR);
            result.put("retMsg", "利息计划删除异常->" + e);
        }
        return result;
    }
    ...
    public Object updateById(HttpServletRequest request, HttpServletResponse response) {
        Map<String, Object> result = new HashMap<String, Object>();
        JSONObject params = JSONObject.parseObject(decryptUtils.DecryptFromRequest(request));// 解析信息
        logger.info("利息计划更新:" + JSONObject.toJSONString(params));
        try {
            iterestService.updateById(params);
            result.put("retCode", AccountConstant.SUCCESS);
            result.put("retMsg", "利息计划更新成功");
        } catch (BizException e) {
            logger.error("利息计划更新异常", e);
            result.put("retCode", AccountConstant.ERROR);
            result.put("retMsg", "利息计划更新异常->" + e.getMsg());
        } catch (Exception e) {
            logger.error("利息计划更新异常", e);
            result.put("retCode", AccountConstant.ERROR);
            result.put("retMsg", "利息计划更新异常->" + e);
        }
        return result;
    }
    ...
    public Object insert(HttpServletRequest request, HttpServletResponse response) {
        Map<String, Object> result = new HashMap<String, Object>();
        JSONObject params = JSONObject.parseObject(decryptUtils.DecryptFromRequest(request));// 解析信息
        logger.info("利息计划新增:" + JSONObject.toJSONString(params));
        try {
            iterestService.insert(params);
            result.put("retCode", AccountConstant.SUCCESS);
            result.put("retMsg", "利息计划新增成功");
        } catch (BizException e) {
            logger.error("利息计划新增异常", e);
            result.put("retCode", AccountConstant.ERROR);
            result.put("retMsg", "利息计划新增异常->" + e.getMsg());
        } catch (Exception e) {
            logger.error("利息计划新增异常", e);
            result.put("retCode", AccountConstant.ERROR);
            result.put("retMsg", "利息计划新增异常->" + e);
        }
        return result;
    }
}
